...
Map.addLayer(sentinel2.clip(aoi), s2Viz, 'median composite');
...
var dem = ee.Image('USGS/3DEP/10m')
      .reproject({crs: sentinel2.projection(), scale: scale})
      .toFloat()  // Convert DEM to Float32 to match Sentinel-2
      .clip(aoi);
...
Export.image.toDrive({
  image: image,  // Convert the final image to Float32 to ensure consistency
  description: 'RI',
  folder: 'DATA',
  shardSize: 512, // Custom shard size
